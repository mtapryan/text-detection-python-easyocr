...
class InsertBankMethodService {
    ...
    public function insertBankMethod($token, $data) {
        $this->logger->log("Starting insert bank method process");
        ...
        $sql = "INSERT INTO bank (bank_name, bank_method, is_active, created_at, modified_at) VALUES (?, ?, ?, ?, ?)";
        if($stmt = mysqli_prepare($this->link, $sql)){
            mysqli_stmt_bind_param($stmt, "ssiss", $bankName, $bankMethod, $isActive, $createdAt, $modifiedAt);
            if(mysqli_stmt_execute($stmt)){
                return json_encode(["status" => "success", "message" => "Bank method inserted successfully"]);
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $service = new InsertBankMethodService($link);
    echo $service->insertBankMethod($jwt, $data);
}
?>