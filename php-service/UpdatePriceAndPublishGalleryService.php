...
class UpdatePriceAndPublishGalleryService {
    private $link;
    private $logger;
    ...
    public function updatePost($token, $data) {
        $this->logger->log("Starting update Gallery Image Price and Publish process");
        ...
        $sql = "UPDATE gallery SET image_price = ?, is_publish = ? WHERE id = ? AND user_id = ?";
        if($stmt = mysqli_prepare($this->link, $sql)){
            mysqli_stmt_bind_param($stmt, "ssss", $imagePrice, $isPublish, $postId, $userId);
            if(mysqli_stmt_execute($stmt)){
                return json_encode(["status" => "success", "message" => "Gallery Image Price and Publish updated successfully"]);
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $service = new UpdatePriceAndPublishGalleryService($link);
    echo $service->updatePost($jwt, $data);
}
?>