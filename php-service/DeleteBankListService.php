...
class DeleteBankListService {
    ...
    public function deleteBankList($token, $data) {
        $this->logger->log("Starting delete bank list process");
        ...
        $sql = "DELETE FROM payment WHERE user_id = ? AND id = ?";
        if($stmt = mysqli_prepare($this->link, $sql)){
            mysqli_stmt_bind_param($stmt, "si", $userId, $id);
            if(mysqli_stmt_execute($stmt)){
                return json_encode(["status" => "success", "message" => "Bank list deleted successfully"]);
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $service = new DeleteBankListService($link);
    echo $service->deleteBankList($jwt, $data);
}
?>