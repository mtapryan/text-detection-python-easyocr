...
class LikeFeedService {
    ...
    public function likePost($token, $postId) {
        $this->logger->log("Starting likePost process");
        ...
        $sql = "UPDATE post SET love = love + 1 WHERE id = ?";
        if($stmt = mysqli_prepare($this->link, $sql)){
            mysqli_stmt_bind_param($stmt, "s", $postId);
            if(mysqli_stmt_execute($stmt)){
                return json_encode(["status" => "success", "message" => "Post liked successfully"]);
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $service = new LikeFeedService($link);
    echo $service->likePost($jwt, $postId);
}
?>