...
class GetPostService {
    ...
    public function getAllPosts($token, $filter, $page, $size) {
        ...
        $sql = "SELECT post.*, user.full_name, user.email 
                FROM post 
                JOIN user ON post.user_id = user.user_id 
                WHERE post.username LIKE ? 
                LIMIT ?, ?";
        if($stmt = mysqli_prepare($this->link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                return json_encode(["status" => "success", "data" => ["records" => $posts, "total" => $total], "message" => "Posts fetched successfully"]);
            } else {
