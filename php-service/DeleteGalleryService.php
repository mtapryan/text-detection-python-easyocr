...
class DeleteGalleryService {
    ...
    public function deletePost($token, $postId) {
        $this->logger->log("Starting delete Image Gallery process");
        ...
        if($stmt = mysqli_prepare($this->link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                return json_encode(["status" => "success", "message" => "Gallery deleted successfully"]);
            } else {
    ...
    private function deleteImage($imageUrl) {
        $watermarkUrl = str_replace("uploads/", "watermark/", $imageUrl);
        ...
        $watermarkDirectory = dirname($watermarkUrl);
        if (is_dir($watermarkDirectory) && count(glob("$watermarkDirectory/*")) === 0) {
            rmdir($watermarkDirectory);
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $service = new DeleteGalleryService($link);
    echo $service->deletePost($jwt, $postId);
}
?>