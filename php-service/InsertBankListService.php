...
class InsertBankListService {
    ...
    public function insertBankList($token, $data) {
        $this->logger->log("Starting insert bank list process");
        ...
        $sql = "INSERT INTO payment (user_id, bank_id, account_number, created_at, modified_at) VALUES (?, ?, ?, ?, ?)";
        if($stmt = mysqli_prepare($this->link, $sql)){
            mysqli_stmt_bind_param($stmt, "sssss", $userId, $bankId, $accountNumber, $createdAt, $modifiedAt);
            if(mysqli_stmt_execute($stmt)){
                return json_encode(["status" => "success", "message" => "Bank list inserted successfully"]);
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $service = new InsertBankListService($link);
    echo $service->insertBankList($jwt, $data);
}
?>