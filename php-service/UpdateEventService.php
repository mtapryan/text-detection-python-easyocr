...
class UpdateEventService {
    ...
    public function updateEvent($token, $data, $file) {
        $this->logger->log("Starting updateEvent process");
        ...
        if ($file) {
            $newEventImage = $this->uploadImage($file, $eventId);
            if (!$newEventImage) {
        ...
        $sql = "UPDATE event SET event_name = ?, event_address = ?, event_lat = ?, event_lang = ?, event_desc = ?, event_image = ?, modify_at = ? WHERE event_id = ?";
        if($stmt = mysqli_prepare($this->link, $sql)){
            mysqli_stmt_bind_param($stmt, "ssssssss", $eventName, $eventAddress, $eventLat, $eventLang, $eventDesc, $eventImage, $modifyAt, $eventId);
            if(mysqli_stmt_execute($stmt)){
                return json_encode(["status" => "success", "message" => "Event updated successfully"]);
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $data = $_POST;
    $file = isset($_FILES['event_image']) ? $_FILES['event_image'] : null;

    $service = new UpdateEventService($link);
    echo $service->updateEvent($jwt, $data, $file);
}
?>