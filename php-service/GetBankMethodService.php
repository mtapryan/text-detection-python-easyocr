...
class GetBankMethodService {
    ...
    public function getBankMethods($token, $filter, $page, $size) {
        $this->logger->log("Fetching all bank methods");
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM bank WHERE bank_name LIKE ? LIMIT ?, ?";
        if($stmt = mysqli_prepare($this->link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                return json_encode(["status" => "success", "data" => ["records" => $banks, "total" => $total], "message" => "Bank methods fetched successfully"]);
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $service = new GetBankMethodService($link);
    echo $service->getBankMethods($jwt, $filter, $page, $size);
}
?>